...
export async function onIntegrateDomain(domain: string, image: string) {
  const user = await currentUser();
  if (!user) return;
  try {
...
export async function onUpdatePassword(password: string) {
  try {
    ...
    return {
      status: 400,
      message: error?.errors[0]?.message || "Password not updated",
    };
  }
}

export async function onGetCurrentDomainInfo(domain: string) {
  const user = await currentUser();
  if (!user) return;
  try {
...
export async function onDeleteUserDomain(id: string) {
  const user = await currentUser();
...
export const onGetAllHelpDeskQuestions = async (id: string) => {
  try {
    const questions = await db.helpDesk.findMany({
      where: {
        domainId: id,
      },
      select: {
